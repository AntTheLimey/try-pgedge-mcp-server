...
#   $s = irm https://raw.githubusercontent.com/AntTheLimey/try-pgedge-mcp-server/main/install.ps1; & ([scriptblock]::Create($s)) -Demo
#
...
"@
    ...
    Write-Host ""
    Write-Info "Starting demo Postgres with Northwind sample data on port $($script:DemoPort)..."
    ...
    for ($i = 0; $i -lt 24; $i++) {
        try {
            & docker exec pgedge-demo-db pg_isready -U demo -d northwind 2>$null | Out-Null
            if ($LASTEXITCODE -eq 0) {
                Write-Ok "Demo database ready (northwind on localhost:$($script:DemoPort))"
